<template>
  <div class="dialog-report">
    ...
    <el-date-picker
        ...
        <el-button type="primary" style="width:10%;margin-left:50%;margin-top:20px;" size="small" @click="handleAddMember()">更新</el-button>
...
export default {
  ...
  methods: {
    ...
    {
      let barId = this.useData.barId
      let activityId = this.useData.activityId
            let id = this.useData.id
      let startTime =  this.useData.startTime;
      let endTime =  this.useData.endTime;
      let order =  this.useData.order;
      let status =  this.useData.status;
      let activityName = this.useData.activityName;
      let type = 'recommend';
      let city = '';
          ...
          switch (this.useData.radio) {
                    case "活动当地":
                           let userId = 0
            let activityId = this.useData.activityId;
           let data1 = await get_detail({userId,activityId});
      ...
      // else
      // {
      let data = ({ activityName:activityName,id:id,barId: barId, activityId: activityId, startTime: startTime, endTime: endTime, order: order, status: status, type: type,city: city});
      console.log(data)
      update_hot_activity(data);
        this.$emit('emitFunc', { function: 'getData', arguments: [] })
        this.handleDialogClose()
