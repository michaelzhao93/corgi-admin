<template>
  <div class="flex align-center">
    <el-tooltip
      ...
      placement="bottom"
      :disabled="!isChecked"
      :content="userInfo.checkDesc"
    >
      <span class="mar-r-10" v-text="userInfo.desc || '--'" />
    </el-tooltip>
...
<script>
import { agreeDesc } from '@/api/contentMgr'
...
export default {
  ...
  computed: {
    isChecked() {
      return this.userInfo.desc === autoCheckText
    }
  },
  methods: {
    async agreeCheck() {
      try {
        ...
        const desc = this.userInfo.checkDesc
    ...
    async failCheck() {
      try {
        await this.$msgbox.confirm(`确定要进行该操作`, `删除用户描述`, {
