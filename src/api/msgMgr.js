...
// 搜索用户
export function get_system_message(params) {
  return request({
...
export function update_system_message(params) {
  //   return request({
  //     url: '/bar/add_bar',
  //     method: 'post',
  // responseType: 'json',
  ...
  const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MzU0MjUwNDgsInVzZXJJZCI6Ii0xIiwidmVyc2lvbiI6IlYxLjAiLCJpYXQiOjE1ODgzMDIxNTV9.UcR_5JX0hIPBzySF9q4vh4oI8DyY7pXmHTzQqffiKkM'
  axios.defaults.headers.common['jwt'] = token;
  axios.post('http://139.224.63.240:7888/system/update_system_message',data)
  .then(res=>{
      console.log('res=>',res);            
  })

}
export function get_message_record(params) {
  return request({
    url: '/system/get_message_record',
    method: 'get',
    params
    })
}
